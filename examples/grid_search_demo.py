"""Synthetic grid-search demo."""

from __future__ import annotations

from pathlib import Path
import sys

import pandas as pd

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from backtester.data.loader import DataLoader
from backtester.engine import BacktestConfig
from backtester.research import run_grid_search
from backtester.strategy import MomentumStrategy


class SyntheticLoader(DataLoader):
    def fetch(self, ticker: str, start: str, end: str) -> pd.DataFrame:
        del ticker, start, end
        dates = pd.date_range("2020-01-01", periods=160, freq="B", name="date")
        closes = [100.0 + index * 0.05 + (index % 25 - 12) * 0.35 for index in range(160)]
        return pd.DataFrame(
            {
                "open": closes,
                "high": [close * 1.01 for close in closes],
                "low": [close * 0.99 for close in closes],
                "close": closes,
                "volume": [100_000] * len(closes),
            },
            index=dates,
        )


def main() -> None:
    results = run_grid_search(
        loader=SyntheticLoader(),
        strategy_factory=MomentumStrategy,
        param_grid={"fast_window": [5, 10], "slow_window": [30, 50]},
        config=BacktestConfig(ticker="SYNTH", start_date="2020-01-01", end_date="2020-08-01"),
    )
    print(results.to_string(index=False))


if __name__ == "__main__":
    main()

